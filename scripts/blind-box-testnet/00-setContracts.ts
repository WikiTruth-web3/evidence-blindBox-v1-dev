...
/**
npx hardhat run scripts/blind-box-testnet/00-setContracts.ts --network sapphire-testnet
 */

// 1. 利用条件类型动态筛选出 AllContracts 中所有支持 "setContracts" 方法的合约名称
import { AllContracts, ContractFunctions } from "../types/call-params";

type ContractsWithSetAddressManager = {
    [C in keyof AllContracts]: "setContracts" extends ContractFunctions<AllContracts[C]> ? C : never;
}[keyof AllContracts];
