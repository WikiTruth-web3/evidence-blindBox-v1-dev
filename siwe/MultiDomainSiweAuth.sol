...
contract MultiDomainSiweAuth is SiweAuthError, A13e {
    ...
    constructor(
        ...
        string[] memory additionalDomains
    ) {
    ...
    function setPrimaryDomain(
        string calldata newPrimaryDomain
    ) external onlyAdmin validDomain(newPrimaryDomain) {
     ...
     */
    function _decodeAndValidateToken(
        bytes memory token
    ...
    function isSessionValid(bytes memory token) external view returns (bool) {
        ...
        try this.decodeAndValidateTokenPublic(token) returns (
            AuthToken memory
        ) {
            return true;
        } catch {
     ...
     * @return The decoded and validated AuthToken structure
     */
    function decodeAndValidateTokenPublic(
        bytes memory token
    ) external view returns (AuthToken memory) {
