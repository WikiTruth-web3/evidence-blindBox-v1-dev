 ...
 *
 */
contract SiweAuth is MultiDomainSiweAuth {
    // =======================================================================================================
    constructor(
     ...
     */
    function getMsgSender(bytes memory token) external view returns (address) {
     ...
     */
    function testStatementVerification(
        bytes memory token,
        string calldata expectedStatement
    ) external view returns (bool) {
        string memory actualStatement = _getStatement(token);
        if (
            keccak256(bytes(actualStatement)) ==
            keccak256(bytes(expectedStatement))
        ) {
            return true;
        }
        return false;
     ...
     */
    function testHasResourceAccess(
        bytes memory token,
        ...
        for (uint256 i = 0; i < tokenResources.length; i++) {
            if (
                keccak256(bytes(tokenResources[i])) ==
