...
/**
 * @notice Local Ethereum test environment uses, ignores sapphire
 * Includes all related code
 */
...
contract TruthBox is TruthBoxBase, ITruthBox {
    ...
    function _setBoxData(
        ...
        // NOTE: Local Ethereum test environment uses, ignores sapphire
        // Generate encrypted nonce (critical fix: save nonce for decryption)
        // bytes32 nonce = bytes32(Sapphire.randomBytes(32, abi.encodePacked(boxId, msg.sender)));
        ...
        _secretData[boxId] = SecretData({
            _minter: msg.sender,
            _nonce: bytes32(0), // nonce
            _encryptedData: key_ // NOTE Local Ethereum test environment uses, ignores sapphire, uses key_ instead of encryptedData
        });
    ...
    function create(
        ...
        unchecked {
            deadline = block.timestamp + 365 days; // 365----15
        }
        ...
        // Log the price and deadline, no need to record status, because status is Storing status
    ...
    function _getStatus(uint256 boxId_) internal view returns (Status) {
        ...
        // If the deadline has passed, then we need to check the status of the box
        if (_publicData[boxId_]._deadline < block.timestamp) {
            // 1, If the box is in Selling/Auctioning, and there is no buyer, then it is Published status
     ...
     * NOTE: Local Ethereum test environment uses, ignores siweAuth token verification, so commented out some content
     */
    ...
    function getPrivateData(
        ...
        // NOTE: Local Ethereum test environment uses, ignores sapphire, uses key_ instead of encryptedData
    ...
    function _isDeadlineIn30days(uint256 boxId_) internal view {
        ...
        if (
            deadline < block.timestamp || deadline > block.timestamp + 30 days
        ) {
    ...
    function extendDeadline(uint256 boxId_, uint256 time_) external {
        ...
        if (time_ > 365 days) revert InvalidPeriod(); // 365----15
    ...
    // Safe payment, Box must not be public and invalid
