...
- **calcPayMoney（计算需要支付的资金）**：这是一个 View 类型的函数用于前端展示，如果竞拍失败`bidders.includ(user) && buyer !== user`，并且想再次竞拍时，就可以通过该函数查看自己要支付多少资金（计算方式为：`box.price - orderAmount`）。
...
function OrderAmountsWithdraw(address token_,uint256[] calldata list_, address user_) external;
function RefundAmountsWithdraw(address token_,uint256[] calldata list_, address user_) external;
function RewardsWithdraw(address token_, address user_) external;
```
...
2.  **Service Fee**: 平台收取的手续费，转入 DAO Treasury。
