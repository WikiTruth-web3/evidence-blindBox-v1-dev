...
**合约部署模块**
- 负责部署所有智能合约
- 包括：AddressManager、代币合约、NFT合约、交换合约等
- 返回签名者和合约实例

### 📁 connectors.js  
**连接器模块**
- 为不同用户角色创建合约连接器实例
- 包括：TruthBox、TruthNFT、Exchange、FundManager等连接器
- 按功能分类组织连接器
...
```javascript
const { deployContracts, configureTokens, initializeContracts } = require("./fixtures");
