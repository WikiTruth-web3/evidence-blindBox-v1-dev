...
// npx hardhat test test/contracts-eth/Swap2.js 
...
describe("交易测试-多角色参与交易过程", function () {
  ...
  it("1-过期出售-seller-buyer-completer-提取资金", async function () {
    const { admin, dao, minter, seller, buyer, settlementToken, completer, blindBox, 
      ...
      userManager_completer,userId_completer, userId_minter, userId_seller,
    ...
    // 
    // seller出售 会增加额外费率
    await exchange_seller.sell(1, address_zero, settlementToken_amount("2000"));
    await exchange_seller.auction(2, address_zero, settlementToken_amount("2000"));
    // ========================== 购买 ==========================
    ...
    expect(await exchange.completerIdOf(1)).to.equal(userId_completer);
    expect(await exchange.completerIdOf(2)).to.equal(userId_completer);
    ...
    expect(incomeMinter).to.equal(settlementToken_amount("1900"));
    // expect(serviceFee).to.equal(100);

    expect(incomeCompleter).to.equal(settlementToken_amount("20"));
    expect(incomeSeller).to.equal(settlementToken_amount("20"));
    // NOTE 服务费 3%
    // 查看合约的余额，已支付60服务费
    const balancefundManager = await settlementToken.balanceOf(fundManager.target);
    expect(balancefundManager).to.equal(settlementToken_amount("100000001940"));
    ...
    expect(balanceMinter02-balanceMinter).to.equal(settlementToken_amount("1900"));
    ...
    await fundManagerSeller.withdrawRewards(settlementToken.target, seller.address);
    ...
    await fundManager_completer.withdrawRewards(settlementToken.target, completer.address);
    const balanceCompleter02 = await settlementToken.balanceOf(completer.address);
    expect(balanceCompleter02-balanceCompleter).to.equal(settlementToken_amount("20"));
    ...
    // 两个box，一共60
    expect(balanceDao_fund_manager1).to.equal(settlementToken_amount("60"));
    ...
    // 资金已经分配完毕
    expect(balancefundManager01).to.equal(settlementToken_amount("100000000000")); 
  ...
  it("2- seller 使用 other Token出售", async function () {
    ...
    await time.increase(380*24*60*60)
    await exchange_seller.sell(1, wBTC.target, wBTC_amount("2000"));
    await exchange_seller.auction(2, wBTC.target, wBTC_amount("2000"));
